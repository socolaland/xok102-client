...
cc.Class({
    ...
    getDataPage(page, limit = 5) {
        HttpRequest.Get(
            `${cc.CORE.CONFIG.SERVER_API}/transfer/transfer-list`, 
            {
                page: page,
                limit: limit,
    ...
    onData: function (dataHistory) {
        ...
        if (void 0 != dataHistory.result) {
            ...
            data.forEach(item => {
                ...
                const nodeComp = itemNode.getComponent("Lobby.Popup.History_Transfer.Item.Controller");
                nodeComp.init(this, item);
                nodeComp.time.string = "Thời gian: " + cc.CORE.UTIL.getStringDateByTime(item.createdAt);

                nodeComp.username.string = item.receiver_nickname;
                ...
                nodeComp.amount.string = cc.CORE.UTIL.numberWithCommas(item.amount);
